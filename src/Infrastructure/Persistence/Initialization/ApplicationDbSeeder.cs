using Belsio.Erp.Infrastructure.Identity;
using Belsio.Erp.Infrastructure.Multitenancy;
using Belsio.Erp.Infrastructure.Persistence.Context;
using Belsio.Erp.Shared.Authorization;
using Belsio.Erp.Shared.Multitenancy;
...
internal class ApplicationDbSeeder
{
    ...
    {
        foreach (string roleName in BaseRoles.DefaultRoles)
        {
            ...
            // Assign permissions
            if (roleName == BaseRoles.Basic)
            {
                await AssignPermissionsToRoleAsync(dbContext, BasePermissions.Basic, role);
            }
            else if (roleName == BaseRoles.Admin)
            {
                await AssignPermissionsToRoleAsync(dbContext, BasePermissions.Admin, role);

                if (_currentTenant.Id == MultitenancyConstants.Root.Id)
                {
                    await AssignPermissionsToRoleAsync(dbContext, BasePermissions.Root, role);
                }
            }
        }
    }
    ...
    private async Task AssignPermissionsToRoleAsync(ApplicationDbContext dbContext, IReadOnlyList<FSHPermission> permissions, ApplicationRole role)
    {
        ...
        {
            if (!currentClaims.Any(c => c.Type == BaseClaims.Permission && c.Value == permission.Name))
            {
                ...
                {
                    RoleId = role.Id,
                    ClaimType = BaseClaims.Permission,
    ...
    private async Task SeedAdminUserAsync()
    {
        ...
        {
            string adminUserName = $"{_currentTenant.Id.Trim()}.{BaseRoles.Admin}".ToLowerInvariant();
            adminUser = new ApplicationUser
            {
                FirstName = _currentTenant.Id.Trim().ToLowerInvariant(),
                LastName = BaseRoles.Admin,
        ...
        // Assign role to user
        if (!await _userManager.IsInRoleAsync(adminUser, BaseRoles.Admin))
        {
            _logger.LogInformation("Assigning Admin Role to Admin User for '{tenantId}' Tenant.", _currentTenant.Id);
            await _userManager.AddToRoleAsync(adminUser, BaseRoles.Admin);
        }
    }
}